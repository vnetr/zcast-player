...
logEventStart(args: Partial<EventRow['data']> & { event_id: string }) {
 ...
 logEventCompleted(args: Partial<EventRow['data']> & { event_id: string; duration?: number }) {
  const { event_id, duration, timestamp, status, actions, ...rest } = args;
  const mergedActions = actions ? Array.from(new Set([...actions, 'finished'])) : ['started', 'finished'];
  const row: EventRow = {
    kind: 'events',
    data: {
      event_id,
    ...
    window.addEventListener('beforeunload', () => {
      ...
      try {
        ...
        if (slice.length && 'sendBeacon' in navigator) {
          const blob = new Blob([JSON.stringify({ items: slice })], { type: 'application/json' });
  ...
  private async flushOnce() {
    ...
    try {
      ...
      // network or other → retry with backoff
  ...
  private async handleRetry(batch: Array<{ row: Row; tries: number }>, status: number) {
    // Simple policy: increment tries for each item in the failed batch
    for (let i = 0; i < batch.length; i++) {
      const idx = i; // relative to start of queue
      if (this.queue[idx]) this.queue[idx].tries = (this.queue[idx].tries ?? 0) + 1;
    }
    ...
    const delay = Math.min(15000, this.backoffBaseMs * Math.pow(2, Math.min(triesMax, 6)) + jitter);
...
export function makeEventId(player: string, docId: string | number, ts: number) {
  // Stable, readable ID — upsert happens server-side by this key
  return `evt-${player}__${docId}__${ts}`;
}
export function makeDailyKey(prefix: string, key: string, d: Date = new Date()) {
